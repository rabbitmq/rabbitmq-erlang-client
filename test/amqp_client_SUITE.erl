...
connection_multi_close_test_()          -> ?RUN([]).
channel_close_normal_test_()            -> ?RUN([]).
connection_close_normal_test_()         -> ?RUN([]).
basic_ack_test_()                       -> ?RUN([]).
...
connection_errors_test_()               -> ?RUN([negative, {timeout, 30}]).
channel_errors_test_()                  -> ?RUN([negative]).
