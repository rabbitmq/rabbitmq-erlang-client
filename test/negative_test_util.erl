...
connection_errors_test() ->
    io:format("Running connections test~n"),
    Errors = [test_util:new_connection([{virtual_host, <<"/non-existing">>}]),
              test_util:new_connection([{virtual_host, <<"/no-access">>}]),
              test_util:new_connection(just_network, [{password, <<"mh">>}])],
    error_logger:info_msg("Errors: ~p~n", [Errors]),
    ?assertMatch([{error, access_refused},
                  {error, access_refused},
                  {error, auth_failure}], Errors).
